...
describe('Commands', () => {
  ...
  describe('toggleTabs', () => {
    it('deep clones input to avoid mutation errors', () => {
      const actionParams = { hidden: true };
      uut.toggleTabs('theContainerId', actionParams);
      expect(mockCommandsSender.toggleTabs.mock.calls[0][1]).not.toBe(actionParams);
    });

    it('passes options for container', () => {
      uut.toggleTabs('theContainerId', { to: 'hidden' });
      expect(mockCommandsSender.toggleTabs).toHaveBeenCalledTimes(1);
      expect(mockCommandsSender.toggleTabs).toHaveBeenCalledWith('theContainerId', { to: 'hidden' });
    });
  });
